...
# 安装 python3-venv（检测 Python 版本，Ubuntu 需要 python3.X-venv）
if ! $PYTHON -m venv --help &>/dev/null 2>&1; then
    echo "[安装] python3-venv..."
    PY_VER=$($PYTHON -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')")
    apt-get update -qq 2>/dev/null || apt-get update 2>/dev/null
    INSTALLED=0
    for pkg in "python${PY_VER}-venv" "python3-venv"; do
        echo "  -> 尝试: $pkg"
        if apt-get install -y -qq --fix-missing "$pkg" 2>/dev/null; then
            echo "  OK: $pkg"
            INSTALLED=1
            break
        fi
    ...
    if [ "$INSTALLED" = "0" ] || ! $PYTHON -m venv --help &>/dev/null 2>&1; then
        echo "[错误] python3-venv 安装失败"
        echo "  请手动执行: apt install python${PY_VER}-venv"
        exit 1
    fi
fi
